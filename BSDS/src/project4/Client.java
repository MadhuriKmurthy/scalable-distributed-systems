...
public class Client {
	...
	private static IMap rmiInterface;
	...
	public static void main(String[] args) throws NotBoundException, IOException {
		...
		try {
			...
			rmiInterface = (IMap) Naming
					.lookup("rmi://" + inputIPAddress + ":" + serverPort + "/MyKeyValueMap" + serverPort);
		...
		try {
			...
			if ("PUT".equalsIgnoreCase(commands[0])) {
				...
				if (commands.length != 3) {
					serverResponse = "Please enter key and value for PUT";
				} else {
				...
				if (commands.length != 2) {
					serverResponse = "Please enter key for GET";
				} else {
				...
				if (commands.length != 2) {
					serverResponse = "Please enter key to DELETE";
				} else {
	...
	private static FileWriter createFileWriter() {
		...
		try {
			...
			String fileName = currentDirectory + "\\PaxosClientLog.txt";

			File clientLogFile = new File(fileName);

			if (!clientLogFile.exists()) {
				clientLogFile.createNewFile();
				System.out.println("Paxos client log file created: " + fileName);
			} else {
				System.out.println("Paxos client log file already exists at : " + fileName);
			}

			fileWriter = new FileWriter(clientLogFile, true);

		} catch (IOException e) {
			System.err.println("Error creating Paxos client log file!");
		}
		return fileWriter;
	}
