...
public class Client {
	...
	private static RMIInterface2 rmiInterface;
	...
	public static void main(String[] args) throws NotBoundException, IOException {
		...
		try {
			...
			rmiInterface = (RMIInterface2) Naming.lookup("rmi://" + inputIPAddress + ":" + serverPort + "/MyKeyValueMap");
	...
	private static FileWriter createFileWriter() {
		...
		try {
			...
			String fileName = currentDirectory + "\\2PCClientLog.txt";

			File clientLogFile = new File(fileName);

			if (!clientLogFile.exists()) {
				clientLogFile.createNewFile();
				System.out.println("2PC client log file created: " + fileName);
			} else {
				System.out.println("2PC client log file already exists at : " + fileName);
			}

			fileWriter = new FileWriter(clientLogFile, true);

		} catch (IOException e) {
			System.err.println("Error creating 2PC client log file!");
		}
		return fileWriter;
	}
